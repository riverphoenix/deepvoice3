...
def get_batch(config):
    ...
    with tf.device('/cpu:0'):
        ...
        if hp.predict_melograph:
            if hp.predict_world:
                text, texts_test, mel, done, mag, magmel, realmel, imagmel, freq, pitch, harmonic, aperiodic = tf.train.slice_input_producer([texts,texts_tests, mels, dones, mags, magmels, realmels, imagmels, freqs,pitches,harmonics,aperiodics], shuffle=True)
            else:
                text, texts_test, mel, done, mag, magmel, realmel, imagmel, freq = tf.train.slice_input_producer([texts,texts_tests, mels, dones, mags, magmels, realmels, imagmels, freqs], shuffle=True)
        else:
            if hp.predict_world:
                text, texts_test, mel, done, mag, pitch, harmonic, aperiodic = tf.train.slice_input_producer([texts,texts_tests, mels, dones, mags, pitches,harmonics,aperiodics], shuffle=True)
            else:
                text, texts_test, mel, done, mag = tf.train.slice_input_producer([texts,texts_tests, mels, dones, mags], shuffle=True)
